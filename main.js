...
function scheduling() {
    ...
    if (processPerid.length==2){
        for (let i = 0; i < SCM; i++) {
            if (timer >= tI[0] && timer < tF[0] && tF[0] < tF[1] ) {
    ...
    if (processPerid.length == 3) {
        for (let i = 0; i < SCM; i++) {
            if (timer >= tI[0] && timer < tF[0] && tF[0] < tF[1] && tF[0] < tF[2]) {
                document.querySelector(`#t0${timer + 1}`).bgColor = "#2167cf";
                document.querySelector(`#t0${timer + 1}`).innerText = 1;
                timer++;
                pE[0]++;
            ...
            else if (timer >= tI[1] && timer < tF[1] && tF[1] < tF[2]) {
                document.querySelector(`#t1${timer + 1}`).bgColor = "#cf6721";
                document.querySelector(`#t1${timer + 1}`).innerText = 1;
                timer++;
                pE[1]++;
    ...
    if (processPerid.length == 4) {
        for (let i = 0; i < SCM; i++) {
            if (timer >= tI[0] && timer < tF[0] && tF[0] < tF[1] && tF[0] < tF[2]&&tF[0]<tF[3]) {
                document.querySelector(`#t0${timer + 1}`).bgColor = "#2167cf";
                document.querySelector(`#t0${timer + 1}`).innerText = 1;
                timer++;
                pE[0]++;
            ...
            else if (timer >= tI[1] && timer < tF[1] && tF[1] < tF[2] && tF[1] < tF[3]) {
                document.querySelector(`#t1${timer + 1}`).bgColor = "#cf6721";
                document.querySelector(`#t1${timer + 1}`).innerText = 1;
                timer++;
                pE[1]++;
            ...
            else if (timer >= tI[2] && timer < tF[2] && tF[2] < tF[3]) {
                document.querySelector(`#t2${timer + 1}`).bgColor = "#cfbe21";
                document.querySelector(`#t2${timer + 1}`).innerText = 1;
                timer++;
                pE[2]++;
